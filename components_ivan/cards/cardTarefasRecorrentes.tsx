...
interface CardTarefasRecorrentesProps {
  navigation: CardTarefasRecorrentesNavigationProp;
}
...
const CardTarefasRecorrentes: React.FC<CardTarefasRecorrentesProps> = ({ navigation }) => {
  ...
  const carregarTarefas = async (filtrosCustom?: Filtros) => {
    ...
    try {
      ...
      // Aplicar todos os filtros de validação para tarefas RECORRENTES
      const tarefasFiltradas = dadosTarefas
        .filter(tarefa => {
          // 1. DEVE ser recorrente (diferença principal)
      ...
      Alert.alert('Erro', 'Não foi possível carregar as tarefas recorrentes');
    } finally {
      setLoading(false);
    }
  };
          ...
          <View style={styles.tarefaInfo}>
            ...
            {item.recorrencia && (
              <View style={styles.recorrenciaContainer}>
                <Text style={styles.recorrenciaIcon}>🔄</Text>
                <Text style={styles.recorrenciaTexto}>
                  {RECORRENCIA_LABELS[item.recorrencia]}
                </Text>
              </View>
            )}
          </View>
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyTitle}>Nenhuma tarefa recorrente encontrada</Text>
      <Text style={styles.emptyMessage}>
        Que tal criar sua primeira tarefa recorrente?
      </Text>
      <TouchableOpacity
        style={styles.criarTarefaButton}
        onPress={handleCriarTarefa}>
        <Text style={styles.criarTarefaButtonText}>➕ Criar Tarefa Recorrente</Text>
      </TouchableOpacity>
    </View>
  );
      ...
      <View style={styles.tarefasContainer}>
        {loading ? (
          <View style={styles.loadingContainer}>
            <Text style={styles.loadingText}>Carregando tarefas recorrentes...</Text>
          </View>
        ) : tarefas.length === 0 ? (
          renderEmptyState()
        ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.filtroModal}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Filtros - Tarefas Recorrentes</Text>
                ...
                <View style={styles.filtroOptionsRow}>
                  {Object.entries(RECORRENCIA_LABELS).map(([key, label]) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.filtroOption,
                        filtros.recorrencia === key && styles.filtroOptionSelected,
                      ]}
                      onPress={() =>
                        setFiltros(prev => ({
                          ...prev,
                          recorrencia: prev.recorrencia === key ? undefined : key,
                        }))
                      }>
                      <Text
                        style={[
                          styles.filtroOptionText,
                          filtros.recorrencia === key && styles.filtroOptionTextSelected,
                        ]}>
                        {label}
                      </Text>
                    </TouchableOpacity>
...
export default CardTarefasRecorrentes;
